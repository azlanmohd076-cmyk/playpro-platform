...
-- Production target verified during this work: playpro2.
...
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1 FROM public.match_results mr WHERE mr.fixture_id = p_fixture_id
  ...
  SET match_state = 'finalized',
      updated_at = now()
  WHERE id = p_fixture_id
    AND match_state = 'ended';
  ...
  IF NEW.card_type IN ('red', 'second_yellow') THEN
    ...
    SELECT c.id,
           CASE WHEN NEW.card_type = 'red'
                ...
                ELSE c.second_yellow_suspension_matches
           END
    ...
    IF v_club_id IS NULL THEN
      SELECT p.club_id INTO v_player_club
    ...
    INSERT INTO public.suspensions (
      ...
      CASE WHEN NEW.card_type = 'second_yellow'
        ...
        ELSE 'Automatic suspension for red card on ' || NEW.match_date::TEXT
      END,
      true,
      NEW.created_by
    )
...
BEGIN
  IF (TG_OP = 'INSERT' AND NEW.is_official = true)
    ...
    SET matches_served = LEAST(s.matches_suspended, s.matches_served + 1),
        ...
        completed_at = CASE
          WHEN (s.matches_served + 1) >= s.matches_suspended THEN now()
          ELSE s.completed_at
        END,
        updated_at = now()
