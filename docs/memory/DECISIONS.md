...
### `DEC-042` — **PELANTIKAN OBSERVER** (Owner, langsung 2026-09-10 malam 9)

**Keputusan Owner (dinyatakan terus, bukan melalui CEO):**

1. Authoriti untuk menulis data sesuatu perlawanan diberikan kepada **2 orang "admin" yang dilantik oleh organisator** untuk perlawanan itu.
2. Bilangan pelantikan **tidak dihadkan kepada 2** — organisator boleh melantik seberapa ramai yang perlu, kerana sehari boleh ada **lebih 8 perlawanan** dan seorang boleh dilantik ke lebih daripada satu perlawanan.
3. Pelantikan ialah **per-perlawanan** (skop), bukan **per-musim** atau **per-kelab** sahaja.

**Yang disokong oleh live (R3):** `record_match_event()` menyemak "pelantikan observer" + "skop observer"; `match_observer_scope_for()` wujud; jadual `match_admin_assignments` wujud. ⇒ model Owner **sudah dijangka** oleh reka bentuk DB.

**Yang belum ada:** `match_observer.html` mempunyai **0** konsep pelantikan/pasangan (ia perakam solo); dan **definisi `match_admin_assignments` TIADA di Git** (0 padanan dalam `database/*.sql` + `supabase/`).

**Lalai selamat yang CTO tetapkan (kerana soalan itu belum dijawab, dan CTO tidak menunggu):**

- "perlu 2 orang" ditafsir sebagai **minimum 2**; lebih dari 2 dibenarkan sebagai pembantu/pengganti. Kalau Owner mahu **tepat 2 (keras)**, kata "tepat" → kekangan DB berubah (`WO-39`).
- Semakan bilangan & kelayakan pelaksana di **lapisan RPC** (`start_match`/`record_match_event`/`finalize_match`), **bukan** di skrin. Alasan: `DEC-004` beku → skrin dan "status" organisator **tidak pernah** menjadi sempadan keselamatan.
- Autoriti pelantikan datang daripada **capability organisator** (model Fasa A, `DEC-033`), **bukan** daripada `profiles.role` mahupun kelulusan dokumen (`DEC-004`).
- Kapasiti harian (8+ perlawanan/hari) direkod sebagai **keperluan bukan fungsian**: satu pegawai boleh memegang berbilang pelantikan; sistem tidak boleh mengunci seorang kepada satu perlawanan pada satu masa.

**Perlu CEO sahkan (konsistensi, bukan kebenaran baharu):** sama ada "2 pegawai" ialah syarat **untuk memulakan** perlawanan atau syarat **untuk mengesahkan/menamatkan** keputusan (finalise) — ia menentukan di RPC mana pemeriksaan diletak. Ditanya sebagai `WO-39`, **tidak dibuka** fasa skema dengannya.

**Tiada apa-apa dilaksanakan** berikutan `DEC-042`: tiada jadual, tiada kekangan, tiada UI. Ia masuk senarai reka bentuk fasa skema.
## K. Log append-only
...
| 2026-09-10 (malam 9) | **R3 mantan CTO masuk + 4 permintaan CTO terjawab**: `match_events`/`player_match_stats`/`match_results`/`standings` = **0 baris** · **0 materialized view** · badan `record_match_event()` = gateway sah (masalah: **UI tidak memanggil**) · badan `register_my_player()` = rujukan KERAS ke lajur tiada → pendaftaran pemain **pasti gagal** · **`trg_official_result_standings` ADA** (standings derive dari `match_results`!) · 12 nama fungsi masih belum disenaraikan. `DRIFT-021` (alat diagnosis menguji objek yang tiada) + `DRIFT-022` (live = Fasa1+Fasa4 separuh+Fasa3, **tanpa** Fasa6/7) direkod. **`DEC-042` diterima terus daripada Owner** (observer = 2 pegawai dilantik organisator per perlawanan; boleh ramai; 8+ perlawanan/hari). `DEC-041` dibuka kepada **opsyen (C)** sebagai `PROPOSED` → CEO+Owner. `WO-33` tertutup, `WO-36` separuh, `WO-37`…`WO-39` dibuka. PR #5 disahkan R3: docs-only, base `23569ac`; CTO ukur `mergeable=MERGEABLE`, `mergeStateStatus=CLEAN`. Tiada DDL; `public/`/`src/`/`tests/` tidak disentuh; ujian 5/4/1 |
