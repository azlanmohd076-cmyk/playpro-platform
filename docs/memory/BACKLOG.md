...
| **WO-16** | Butiran dasar `R-03a`: pelulus dokumen LANJUT, tempoh sah laku, tindakan tamat, hak rayuan | ✅ **SELESAI (H1 / `DEC-031`)** | Baki dua sahaja: ejaan kanonik status (`INELIGIBLE_EXPIRED`, bukan `INELEGIBLE_EXPIRED`) dan pilihan sama ada ia menggantikan `EXPIRED` yang sedia disenarai. Kedua-dua jawapan 1 baris dari CEO → `BOUNDARIES.md` §1 dibetulkan sebagai pembetulan konsistensi |
| **WO-17** | Simpanan dokumen pengesahan (bucket + polisi akses + retensi) untuk tahap LANJUT | 🟡 **DITANGGUH sehingga fasa SKEMA** | Keperluan direkod dalam `DEC-029`; **imej dokumen DILARANG** muncul dalam public view (`PHASE2C P1-9`, `DEC-019`). `DEC-031(b)` tambah: **tempoh simpan 1 tahun kalendar** → perlu dasar padam/auto-luput, bukan simpan selamanya |
...
|---|---|---|---|
| **J1** | `"satu tahun kalendar"` = **12 bulan dari tarikh kelulusan** ATAU **tamat 31 Disember tahun kelulusan**? | CEO | Menentukan kaedah kira luput. Tafsir kedua boleh gugurkan pemain di tengah musim jika dokumen dilulus pada Nov/Dis |
| **J2** | Sahkan nama kanonik: **`INELIGIBLE_EXPIRED`** (ejaan betul) menggantikan `EXPIRED` dalam `BOUNDARIES.md` §1? (ya/tukar) | CEO | CTO engkar tanam typo `INELEGIBLE_EXPIRED` dalam enum; dan satu keadaan tak boleh ada dua nama |
| **J3** | Landasan dev untuk langkah 7 Fasa 2C (`WO-20`): **unpause `playpro1` + pause `playpro` legacy** (cadangan CTO), ATAU Postgres setempat, ATAU beli plan/branch? | **Owner** | Tiada DDL boleh diuji tanpa landasan. Cadangan CTO sekaligus menutup pendedahan 5 jadual RLS-DISABLED pada legacy |
