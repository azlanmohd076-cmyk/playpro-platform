...
| **WO-28** | 🔴 **P0: sahkan sama ada Match Observer menyimpan ke DB atau tidak.** `finaliseMatch()` (`match_observer.html:1120`) hanya panggil `simulateDBWrite()` → `{ok:false,reason:'no_fixture'}`, dan override sebenar (`dashboard_integration.js`) **404** di produksi. **Cara akses (penting — `index.html` TIDAK ada pautan ke halaman ini):** taip terus `https://playpro-platform.vercel.app/match_observer.html` ATAU buka `coach_command_center.html` → klik "Open Match Observer" (L218/L324). Kemudian finalise satu perlawanan ujian dan lihat `match_events` bertambah atau tidak. Toast `✅ Match complete — DNA & Passport updated` keluar **walaupun tiada penulisan** | 🔴 **SEGERA — ujian manual 5 minit oleh Owner (saya tak boleh melayari live)** | `PEER_REVIEW_2026-09-10.md` §9 `DRIFT-013`. Kalau betul: **statistik padang tidak pernah masuk DB**, dan itu mendahului semua kerja fasa skema dari segi kesan kepada pengguna |
| **WO-29** | Jangan sesekali "baiki 404" dengan menyalin `js/dashboard_integration.js` → `public/js/` tanpa pembetulan skema. `MatchRepo.saveEvents()` menulis 6 lajur **yang tiada** di `match_events` live dan meninggalkan `sequence`/`recorded_by`/`voided_*`; ia juga melangkau RPC observer | ⛔ **LARANGAN (peraturan, bukan kerja)** | `DRIFT-014`. Ini perangkap paling mudah buat AI akan datang: nampak macam "satu baris path" padahal mengaktifkan arkitektur lama yang bercanggah dengan Fasa-3 |
| **WO-30** | **Sahinkan pemilikan ref `muirhenvjruvfxenoaxm.supabase.co`** — projek yang mana (`playpro2` ke `playpro`)? **10 saat, tanpa SQL:** di Supabase dashboard lihat baris alamat `.../dashboard/project/<ref>`; yang mana satu bernilai `muirhenvjruvfxenoaxm` itulah DB apl. Anon key `index.html:15` sudah dinyahkod dan mengesahkan apl memang mengarah ke ref itu (bukan placeholder). + baiki laluan hidang 3 skrip / pindahkan fail ke `public/js/` **sebagai satu keputusan reka bentuk** (bukan tampal satu-satu) | 🟡 **BLOCKED — keputusan Owner + fasa PELAKSANAAN** | URL ditulis tetap di 11 fail (tiada laluan konfigurasi). Kalau ref itu = `playpro` **legacy**, maka yang kita semak di `playpro2` mungkin bukan DB yang apl guna — itu perlu diketahui **sebelum** baseline |
| **WO-31** | `connection_test.html:15` + `p0_auth_doctor.html` lalai ke `https://xxxx.supabase.co` (placeholder) → alat diagnosis yang ditulis untuk menangkap drift ini **sendiri rosak** | 🟢 **KECIL — doc/config, boleh masuk PR #6** | 2 baris. Keutamaan rendah tetapi ia sebab drift kekal tak noticed selama 2 bulan |
