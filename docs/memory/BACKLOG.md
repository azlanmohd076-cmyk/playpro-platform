...
| **WO-17** | Simpanan dokumen pengesahan (bucket + polisi akses + retensi) untuk tahap LANJUT | 🟠 **KEPERLUAN CUKUP TEPAT untuk direka; pelaksanaan di fasa SKEMA** | Tiga kekang wajib: **tiada auto-delete** (lifecycle bucket mesti larang pemadaman) · akses dokumen `expired` **lebih ketat** daripada yang masih sah · **imej dokumen DILARANG** dalam public view (`PHASE2C P1-9`, `DEC-019`). Baki: `WO-22` |
| **WO-21** | **Notifikasi sistem** apabila pengesahan hampir/telah luput (`DEC-031(c)`) — saluran & mekanisme | 🟡 **DITANGGUH sehingga fasa SKEMA/I** | INAPP (footer `INBOX` shell terkunci) vs e-mel (infra + PII + sender). CTO cadangkan **in-app dahulu**: tiada kebergantungan luar, dan `WO-13` (redirect masih ke `v0.app`) belum selesai |
